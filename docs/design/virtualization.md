...
| Dragonball | `configuration-dragonball.toml` (default) |

> **Note:** Configuration files are typically installed in `/opt/kata/share/defaults/kata-containers/` or `/usr/share/defaults/kata-containers/`.
...
$ kata-manager list-hypervisors

# Switch to a different hypervisor
$ sudo kata-manager set-hypervisor <hypervisor-name>
```
