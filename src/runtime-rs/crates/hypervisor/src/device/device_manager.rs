...
impl DeviceManager {
        ...
        if let Err(e) = result {
            match device_guard.get_device_info().await {
                ...
                DeviceType::Vfio(device) => {
                    if device.config.dev_type == "b" {
                        // safe here, one virt_path is indeed there when vfio dev_type is `b`.
                        self.shared_info
    ...
    async fn new_device(&mut self, device_config: &DeviceConfig) -> Result<String> {
        ...
        let dev: ArcMutexDevice = match device_config {
            DeviceConfig::BlockCfg(config) => {
                // we'll try to find the device, found and just return id.
    ...
    async fn create_block_device(
        ...
        // generate virt path
        if let Some(virt_path) = self.get_dev_virt_path("b")? {
