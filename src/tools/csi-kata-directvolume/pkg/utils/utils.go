...
func getFileCapacity(filePath string) int64 {
...
func CanDoBindMount(mounter *SafeMountFormater, targetPath string) (bool, error) {
	...
	if err != nil {
		if _, err = MkPathIfNotExit(targetPath); err != nil {
			return false, fmt.Errorf("target path %s checked failed with error: %w", targetPath, err)
		} else {
			notMnt = true
		}
	}
...
func CreateDirectBlockDevice(volID, capacityInBytesStr, storagePath string) (*string, error) {
	...
	if _, err = diskfs.Create(devicePath, capacityInBytes, diskfs.Raw, diskfs.SectorSizeDefault); err != nil {
		errMsg := errors.New(fmt.Sprintf("diskfs create disk failed: %v", err))
	...
	if _, err = os.Stat(devicePath); err != nil {
		klog.Warning("device created failed, with error: %v", err.Error())
		return nil, err
	}
...
func RemoveDirectBlockDevice(devicePath string) error {
...
func IsPathEmpty(path string) (bool, error) {
	f, err := os.Open(path)
	if err != nil {
		return true, fmt.Errorf("unable to open path volume, error: %v", err)
	}
	defer f.Close()
