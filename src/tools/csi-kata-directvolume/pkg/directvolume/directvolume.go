...
func (dv *directVolume) createVolume(volID, name string, cap int64, kind string) (*state.Volume, error) {
	...
	err := os.MkdirAll(path, utils.PERM)
	if err != nil {
...
func (dv *directVolume) loadFromVolume(size int64, srcVolumeId, destPath string) error {
	...
	return loadFromFilesystemVolume(directVolume, destPath)
}

func loadFromFilesystemVolume(directVolume state.Volume, destPath string) error {
...
func (dv *directVolume) getAttachCount() int64 {
	count := int64(0)
	for _, vol := range dv.state.GetVolumes() {
		if vol.Attached {
			count++
		}
	}
