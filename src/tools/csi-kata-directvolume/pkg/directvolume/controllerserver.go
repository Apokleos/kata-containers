...
func (dv *directVolume) ListVolumes(ctx context.Context, req *csi.ListVolumesRequest) (*csi.ListVolumesResponse, error) {
	...
	for index := startIdx - 1; index < volumesLength && index < maxLength; index++ {
		directVol = volumes[index]
		healthy, msg := dv.doHealthCheckInControllerSide(directVol.VolID)
		klog.V(3).Infof("Healthy state: %s Volume: %t", directVol.VolName, healthy)
		volumeRes.Entries = append(volumeRes.Entries, &csi.ListVolumesResponse_Entry{
			Volume: &csi.Volume{
				VolumeId:      directVol.VolID,
				CapacityBytes: directVol.VolSize,
			},
			Status: &csi.ListVolumesResponse_VolumeStatus{
				PublishedNodeIds: []string{directVol.NodeID},
	...
	klog.V(5).Infof("List Volumes: %+v", *volumeRes)
...
func (dv *directVolume) ControllerGetVolume(ctx context.Context, req *csi.ControllerGetVolumeRequest) (*csi.ControllerGetVolumeResponse, error) {
	...
	volume, err := dv.state.GetVolumeByID(req.GetVolumeId())
	if err != nil {
		klog.Warningf("ControllerGetVolume reports abnormal volume %v condition: volume is not found", req.GetVolumeId())
	...
	healthy, msg := dv.doHealthCheckInControllerSide(req.GetVolumeId())
