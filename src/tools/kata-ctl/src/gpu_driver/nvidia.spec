...
%global _systemd_util_dir /usr/lib64/systemd

%if 0%{?rhel}
%global _glvnd_libdir   /usr/lib64/libglvnd
%endif
...
mkdir -p %{buildroot}/usr/lib64/vdpau/

%ifarch x86_64 aarch64 ppc64le

mkdir -p %{buildroot}/usr/bin
mkdir -p %{buildroot}%{_datadir}/nvidia/
mkdir -p %{buildroot}/usr/lib64/nvidia/wine/
mkdir -p %{buildroot}/usr/lib64/xorg/modules/drivers/
mkdir -p %{buildroot}/usr/lib64/xorg/modules/extensions/
...
# Binaries
install -p -m 0755 nvidia-{debugdump,smi,cuda-mps-control,cuda-mps-server,bug-report.sh} %{buildroot}/us r

%ifarch x86_64
mkdir -p %{buildroot}%{_dbus_systemd_dir}/
install -p -m 0755 nvidia-powerd %{buildroot}/usr/bin
%endif
...
sed -i -e 's|@LIBDIR@|/usr/lib64|g' %{buildroot}%{_sysconfdir}/X11/xorg.conf.d/99-nvidia-modules.conf
...
sed -i -e 's|@LIBDIR@|/usr/lib64|g' %{buildroot}%{_sysconfdir}/X11/xorg.conf.d/10-nvidia.conf
%endif

# X stuff
install -p -m 0755 nvidia_drv.so %{buildroot}/usr/lib64/xorg/modules/drivers/
install -p -m 0755 libglxserver_nvidia.so.%{version} %{buildroot}/usr/lib64/xorg/modules/extensions/libglxserver_nvidia.so
...
ln -sf /usr/lib64/libnvidia-allocator.so.1 %{buildroot}/usr/lib64/gbm/nvidia-drm_gbm.so
%endif

# NGX Proton/Wine library
%ifarch x86_64
cp -a *.dll %{buildroot}/usr/lib64/nvidia/wine/
%endif

# Unique libraries
cp -a lib*GL*_nvidia.so* libcuda.so* libnv*.so* %{buildroot}/usr/lib64/
cp -a libnvcuvid.so* %{buildroot}/usr/lib64/
cp -a libvdpau_nvidia.so* %{buildroot}/usr/lib64/vdpau/
%ifnarch %{ix86}
cp -a libcudadebugger.so* %{buildroot}/usr/lib64/
%endif
%ifarch x86_64 aarch64
cp -a libnvoptix.so* %{buildroot}/usr/lib64/
cp -a nvoptix.bin %{buildroot}%{_datadir}/nvidia/
%endif

# libglvnd indirect entry point and private libglvnd libraries
%if 0%{?rhel} == 6 || 0%{?rhel} == 7
cp -a libGLX_indirect.so* %{buildroot}/usr/lib64/
...
install -p -m 0755 systemd/nvidia-sleep.sh %{buildroot}/usr/bin/
...
/usr/lib64/libEGL_nvidia.so.%{version}
/usr/lib64/libGLESv1_CM_nvidia.so.1
/usr/lib64/libGLESv1_CM_nvidia.so.%{version}
/usr/lib64/libGLESv2_nvidia.so.2
/usr/lib64/libGLESv2_nvidia.so.%{version}
/usr/lib64/libGLX_nvidia.so.0
/usr/lib64/libGLX_nvidia.so.%{version}
%ifarch x86_64 ppc64le aarch64
/usr/lib64/libnvidia-cfg.so.1
/usr/lib64/libnvidia-cfg.so.%{version}
/usr/lib64/libnvidia-egl-gbm.so.1
/usr/lib64/libnvidia-egl-gbm.so.1.1.0
/usr/lib64/gbm/nvidia-drm_gbm.so
%{_datadir}/egl/egl_external_platform.d/15_nvidia_gbm.json
%endif
/usr/lib64/libnvidia-eglcore.so.%{version}
/usr/lib64/libnvidia-glcore.so.%{version}
/usr/lib64/libnvidia-glsi.so.%{version}
%ifarch x86_64 aarch64
# Raytracing
/usr/lib64/libnvidia-rtcore.so.%{version}
/usr/lib64/libnvoptix.so.1
/usr/lib64/libnvoptix.so.%{version}
%{_datadir}/nvidia/nvoptix.bin
/usr/lib64/libnvidia-ngx.so.1
/usr/lib64/libnvidia-ngx.so.%{version}
%endif
# Wine libraries
%ifarch x86_64
%dir /usr/lib64/nvidia
%dir /usr/lib64/nvidia/wine
/usr/lib64/nvidia/wine/*.dll
%endif
%if 0%{?fedora} || 0%{?rhel} >= 7
/usr/lib64/libnvidia-glvkspirv.so.%{version}
%endif
/usr/lib64/libnvidia-tls.so.%{version}
/usr/lib64/vdpau/libvdpau_nvidia.so.1
/usr/lib64/vdpau/libvdpau_nvidia.so.%{version}
/usr/lib64/libnvidia-allocator.so.1
/usr/lib64/libnvidia-allocator.so.%{version}
%ifnarch %{ix86}
/usr/lib64/libnvidia-api.so.1
%endif
%ifarch x86_64
/usr/lib64/libnvidia-pkcs11*.so.%{version}
%endif

%files cuda-libs
/usr/lib64/libcuda.so
/usr/lib64/libcuda.so.1
/usr/lib64/libcuda.so.%{version}
/usr/lib64/libnvcuvid.so.1
/usr/lib64/libnvcuvid.so.%{version}
/usr/lib64/libnvidia-encode.so.1
/usr/lib64/libnvidia-encode.so.%{version}
/usr/lib64/libnvidia-nvvm.so.4
/usr/lib64/libnvidia-nvvm.so.%{version}
/usr/lib64/libnvidia-opticalflow.so.1
/usr/lib64/libnvidia-opticalflow.so.%{version}
/usr/lib64/libnvidia-opencl.so.1
/usr/lib64/libnvidia-opencl.so.%{version}
/usr/lib64/libnvidia-ptxjitcompiler.so.1
/usr/lib64/libnvidia-ptxjitcompiler.so.%{version}
%ifnarch %{ix86}
/usr/lib64/libnvidia-vulkan-producer.so*
/usr/lib64/libcudadebugger.so.1
/usr/lib64/libcudadebugger.so.%{version}
%endif
%ifarch x86_64
/usr/lib64/libnvidia-wayland-client.so.%{version}
%endif

%files NvFBCOpenGL
%ifnarch ppc64le
/usr/lib64/libnvidia-fbc.so.1
/usr/lib64/libnvidia-fbc.so.%{version}
%endif

%files NVML
/usr/lib64/libnvidia-ml.so.1
/usr/lib64/libnvidia-ml.so.%{version}
